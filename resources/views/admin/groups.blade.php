        ...
        @foreach($groups as $group)
            <div class="col-md-6">
                ...
                @if(env('SEND_MAIL', false) == true)
                    <button type="button" class="btn btn-primary btn-lg btn-block" data-toggle="modal" data-target="#editMailTexts-{{ $group->id }}">Modifica Testi Mail per {{ $group->name }}</button>
                @endif

                <button type="button" class="btn btn-primary btn-lg btn-block load-group-file" data-toggle="modal" data-target="#loadFile" data-group="{{ $group->id }}">Carica File in '{{ $group->name }}'</button>
...
@role('admin')
    ...
    @if(env('SEND_MAIL', false) == true)
        @foreach($groups as $group)
            <div class="modal fade" id="editMailTexts-{{ $group->id }}" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            <h4 class="modal-title">Modifica Testi Mail per {{ $group->name }}</h4>
                        </div>
                        <form method="POST" action="{{ url('admin/mails') }}">
                            ...
                            <div class="modal-body">
                                <div>
                                    <p>
                                        Da qui è possibile configurare diversi testi per le mail di notifica inviate agli utenti del gruppo '{{ $group->name }}', a seconda del nome del file.
                                    </p>
                                    ...
                                    <ul class="nav nav-tabs" role="tablist">
                                        <?php $index = 0 ?>
                                        @foreach(App\MailText::where('group_id', $group->id)->where('fallback', false)->orderBy('created_at', 'asc')->get() as $index => $mt)
                                            <li role="presentation" class="{{ $index++ == 0 ? 'active' : '' }}"><a href="#mailtext-{{ $mt->id }}-{{ $group->id }}" role="tab" data-toggle="tab">{{ $mt->rule }}</a></li>
                                        @endforeach
                                        <li role="presentation" class="{{ $index == 0 ? 'active' : '' }}"><a href="#mailtext-new-{{ $group->id }}" role="tab" data-toggle="tab">Nuova Regola</a></li>
                                        <li role="presentation" class=""><a href="#mailtext-default-{{ $group->id }}" role="tab" data-toggle="tab">Mail di Default</a></li>
                                    </ul>

                                    <div class="tab-content">
                                        <?php $index = 0 ?>
                                        @foreach(App\MailText::where('group_id', $group->id)->where('fallback', false)->orderBy('created_at', 'asc')->get() as $index => $mt)
                                            <div role="tabpanel" class="tab-pane {{ $index++ == 0 ? 'active' : '' }}" id="mailtext-{{ $mt->id }}-{{ $group->id }}">
                                        ...
                                        <div role="tabpanel" class="tab-pane {{ $index == 0 ? 'active' : '' }}" id="mailtext-new-{{ $group->id }}">
                                        ...
                                        <?php $mt = App\MailText::where('group_id', $group->id)->where('fallback', true)->first() ?>
                                        <div role="tabpanel" class="tab-pane" id="mailtext-default-{{ $group->id }}">
